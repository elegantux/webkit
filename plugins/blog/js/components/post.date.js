((webkit) => {
  async function plugin(editor) {
    const COMPONENT_TYPE = 'blog_post_date';
    const COMPONENT_NAME = 'Post Date';
    const DATA_KEY = 'data-wk-type';
    const CATEGORY = 'Blog';
    const API_ENDPOINT = webkit.webkitBackendUrls.backendApiUrl + '?plugin=blog&module=default&action=postDate';
    ...
    const component = {
      ...
      model: {
        defaults: {
          ...
          [TRAITS.DATE_FORMAT.name]: 'Y-m-d\\TH:i',
          traits: [
            TRAITS.TAG_NAME,
            TRAITS.DATE_FORMAT,
          ],
